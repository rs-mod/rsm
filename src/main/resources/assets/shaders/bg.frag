...
void main(void) {
    ...
    vec3 clickColor = vec3(1.0, 1.0, 1.0);
    vec2 clickPosition = vec2(0.5, 0.5); // Posición del clic (normalizada)
    float clickRadius = 0.1; // Radio del área afectada por el clic

    float distanceToClick = distance(p, clickPosition);
    float clickFactor = smoothstep(clickRadius, 0.0, distanceToClick);
