...
public class EventDispatcher {
    ...
    private void init() {
        ...
        LevelRenderEvents.END_EXTRACTION.register((context) -> new Render3DEvent.Extract(context).post());

        LevelRenderEvents.AFTER_TRANSLUCENT_TERRAIN.register((context) -> new Render3DEvent.Last(context).post());

        ClientLevelEvents.AFTER_CLIENT_LEVEL_CHANGE.register((_, world) -> new WorldEvent.Load(world).post());
    ...
    @SubscribeEvent
    private void onChatPacket(PacketEvent.MainReceivePre event, ClientboundSystemChatPacket packet) {
        if (packet.overlay()) {
    ...
    @SubscribeEvent
    private void onTimeUpdate(PacketEvent.MainReceivePre event, ClientboundSetTimePacket packet) {
