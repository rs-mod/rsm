...
public class ColourValueComponent extends ValueComponent<ColourSetting> {
    ...
    public void render(int mouseX, int mouseY, float partialTicks) {
        ...
        RenderUtils.drawRoundedRect(sbX, sbY, width, baseHeight, 1, color);
        ...
        RenderUtils.drawRoundedRect(boxX - 1, boxY - 2, bgwidth, 54, 1, FatalityColors.PANEL);

        renderOverlay(mouseX, mouseY);

        drawRounded(() -> drawSB(boxX, boxY), boxX, boxY, boxSize, boxSize, 1.5);
        drawRounded(() -> drawHue(hueX, boxY), hueX, boxY, hueStripWidth, boxSize, 1.5);
        drawRounded(() -> drawAlpha2(alphaX, boxY, 1.5), alphaX, boxY, hueStripWidth, boxSize, 1.5);

        GL11.glEnable(GL11.GL_DEPTH_TEST);
    }
    private void drawRounded(Runnable run, double x, double y, double width, double height, double radius) {
        // holy frick
        ...
        RenderUtils.drawRoundedRect(x, y, width, height, radius, FatalityColors.SELECTED_BACKGROUND);
